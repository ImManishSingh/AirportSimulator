...
=======
	...
	// The algorithm used assumes different spots on Airport as different states.
	// eg. Runway for landing is "r'",
	// z is buffer of planes in air
	// s is entry point in system
	// wait queue is 'a' and 'b'.
	// r' is landing end of runway
	...
	public AircraftType at = new AircraftType();
	public int bWaitTime, sWaitTime, eWaitTime, eStateSingleConfigWait, sStateSingleConfigWait, gateNo; //  e-state and s-state wait time
																	// for one gate upto 'n' gates.
	public int astatetimer, cstatetimer, dstatetimer, rstatetimer, rdstatetimer; // This timer which is local to each
																					// state
	public String state; // Defines the current state the plane is in.
	public int RunwayNumber; // This is defined if there are multiple runways. (For now, we have only one).
	public static int freedtime; // Holds amount of time required for each individual gate
	...
	public Aircraft() {
		...
		rstatetimer = 0;
		rdstatetimer = 0;
		eStateSingleConfigWait = 0;
		sStateSingleConfigWait = 0;
		freedtime = 0;
